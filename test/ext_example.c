...
#include "lite_pack/lite_pack_ndarray.h"
...
#define MAKE_TEST(N, T)                                                        \
    static void test_##N(unsigned n, T const input[], T output[],              \
                         uint8_t type)                                         \
    {                                                                          \
        unsigned sz = lip_pack_darray_size_type(buffer, n, type);              \
        if (sz == 0) ERROR;                                                    \
        lip_pack_darray_##N##_data(buffer + sz, n, input);                     \
                                                                               \
        uint8_t t = 0;                                                         \
        sz = lip_unpack_darray_size_type(buffer, &n, &t);                      \
        if (sz == 0) ERROR;                                                    \
        if (t != type) ERROR;                                                  \
        lip_unpack_darray_##N##_data(buffer + sz, n, output);                  \
        for (unsigned i = 0; i < n; ++i)                                       \
            if (input[i] != output[i]) ERROR;                                  \
                                                                               \
        /* IN-PLACE */                                                         \
                                                                               \
        sz = lip_pack_darray_size_type(buffer, n, t);                          \
        if (sz == 0) ERROR;                                                    \
        if (t != type) ERROR;                                                  \
        memcpy(output, input, sizeof(*input) * n);                             \
        lip_pack_darray_##N##_data_inplace((unsigned char *)output, n);        \
                                                                               \
        sz = lip_unpack_darray_size_type(buffer, &n, &t);                      \
        if (sz == 0) ERROR;                                                    \
        if (t != type) ERROR;                                                  \
        lip_unpack_darray_##N##_data_inplace((unsigned char *)output, n);      \
        for (unsigned i = 0; i < n; ++i)                                       \
            if (input[i] != output[i]) ERROR;                                  \
    }
...
int main(void)
{
    test_i8(array_size(input_i8), input_i8, output_i8, LIP_DARRAY_INT8);
    test_i16(array_size(input_i16), input_i16, output_i16, LIP_DARRAY_INT16);
    test_i32(array_size(input_i32), input_i32, output_i32, LIP_DARRAY_INT32);
    test_i64(array_size(input_i64), input_i64, output_i64, LIP_DARRAY_INT64);

    test_u8(array_size(input_u8), input_u8, output_u8, LIP_DARRAY_UINT8);
    test_u16(array_size(input_u16), input_u16, output_u16, LIP_DARRAY_UINT16);
    test_u32(array_size(input_u32), input_u32, output_u32, LIP_DARRAY_UINT32);
    test_u64(array_size(input_u64), input_u64, output_u64, LIP_DARRAY_UINT64);

    test_f32(array_size(input_f32), input_f32, output_f32, LIP_DARRAY_F32);
    test_f64(array_size(input_f64), input_f64, output_f64, LIP_DARRAY_F64);
    return 0;
}
