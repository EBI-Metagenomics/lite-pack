...
LIP_API void lip_read_1darray_f32_data(struct lip_file *io, unsigned size,
                                       float arr[]);

LIP_API void lip_read_1darray_f64_data(struct lip_file *io, unsigned size,
                                       double arr[]);
...
static inline void lip_read_1darray_f32_item(struct lip_file *io, float *val)
{
    io->error |= fread(val, sizeof(float), 1, io->fp) != 1;
    io->error |= lip_load_f32((unsigned char *)val, val) == 0;
}

static inline void lip_read_1darray_f64_item(struct lip_file *io, double *val)
{
