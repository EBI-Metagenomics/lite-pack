...
static inline unsigned lip_store_array16(unsigned char buf[], uint16_t size)
{
    buf[0] = __lip_first_byte(LIP_FMT_ARRAY_16);
    return __lip_store_num16(buf + 1, __LIP_NUM16(size)) + 1;
}

static inline unsigned lip_store_array32(unsigned char buf[], uint32_t size)
{
...
{
    if (size <= 0xf)
        return lip_store_fixarray(buf, (uint8_t)size);
    else if (size <= 0xffff)
        return lip_store_array16(buf, (uint16_t)size);
    return lip_store_array32(buf, (uint32_t)size);
}
