...
static inline unsigned __lip_store_f32(unsigned char buf[static 5], float val)
{
    buf[0] = __lip_first_byte(LIP_FMT_FLOAT_32);
    return 1 + lip_store_num32(buf + 1, __LIP_NUM32(val));
}

static inline unsigned __lip_store_f64(unsigned char buf[static 9], double val)
{
