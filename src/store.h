...
static inline void store_u64(uint8_t buf[static 1], unsigned long val)
{
    buf[0] = first_byte(FMT_UINT_64);
    store_u64_data(buf + 1, val);
}

static inline void store_f32(uint8_t buf[static 1], float val)
{
    CHECK_FLOAT_ORDER();
    buf[0] = first_byte(FMT_FLOAT_32);
    store_f32_data(buf + 1, val);
}

static inline void store_f64(uint8_t buf[static 1], double val)
{
    CHECK_FLOAT_ORDER();
    buf[0] = first_byte(FMT_FLOAT_64);
